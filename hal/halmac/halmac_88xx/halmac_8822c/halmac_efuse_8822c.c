...
static u8 efuse_c2h_part = EFUSE_C2H_PART_1;

static enum halmac_cmd_construct_state
efuse_cmd_cnstr_state_8822c(struct halmac_adapter *adapter);

static enum halmac_ret_status
proc_dump_efuse_8822c(struct halmac_adapter *adapter,
		     enum halmac_efuse_read_cfg cfg);

static enum halmac_ret_status
read_hw_efuse_8822c(struct halmac_adapter *adapter, u32 offset, u32 size,
		   u8 *map);

static enum halmac_ret_status
read_log_efuse_map_8822c(struct halmac_adapter *adapter, u8 *map);

static enum halmac_ret_status
proc_pg_efuse_by_map_8822c(struct halmac_adapter *adapter,
			  struct halmac_pg_efuse_info *info,
			  enum halmac_efuse_read_cfg cfg);

static enum halmac_ret_status
dump_efuse_fw_8822c(struct halmac_adapter *adapter);

static enum halmac_ret_status
dump_efuse_drv_8822c(struct halmac_adapter *adapter);

static enum halmac_ret_status
proc_write_log_efuse_8822c(struct halmac_adapter *adapter, u32 offset, u8 value);

static enum halmac_ret_status
proc_write_log_efuse_word_8822c(struct halmac_adapter *adapter, u32 offset,
			       u16 value);

static enum halmac_ret_status
update_eeprom_mask_8822c(struct halmac_adapter *adapter,
			struct halmac_pg_efuse_info *info, u8 *updated_mask);

static enum halmac_ret_status
check_efuse_enough_8822c(struct halmac_adapter *adapter,
			struct halmac_pg_efuse_info *info, u8 *updated_mask);

static enum halmac_ret_status
proc_pg_efuse_8822c(struct halmac_adapter *adapter,
		   struct halmac_pg_efuse_info *info, u8 word_en,
		   u8 pre_word_en, u32 eeprom_offset);

static enum halmac_ret_status
program_efuse_8822c(struct halmac_adapter *adapter,
		   struct halmac_pg_efuse_info *info, u8 *updated_mask);

static void
mask_eeprom_8822c(struct halmac_adapter *adapter,
		 struct halmac_pg_efuse_info *info);
...
/**
 * dump_efuse_map_8822c() - dump "physical" efuse map
...
dump_efuse_map_8822c(struct halmac_adapter *adapter,
		    enum halmac_efuse_read_cfg cfg)
{
	u8 *map = NULL;
	u8 *efuse_map;
	...
	PLTFM_MSG_TRACE("dump_efuse_map_8822c, efuse_map_valid = %d\n", adapter->efuse_map_valid);
	...
	if (efuse_cmd_cnstr_state_8822c(adapter) != HALMAC_CMD_CNSTR_IDLE) {
	...
	*proc_status = HALMAC_CMD_PROCESS_IDLE;
	...
	status = switch_efuse_bank_8822c(adapter, HALMAC_EFUSE_BANK_WIFI);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]switch efuse bank!!\n");
		return status;
	}

	status = proc_dump_efuse_8822c(adapter, cfg);
	if (status != HALMAC_RET_SUCCESS) {
	...
	if (cnv_efuse_state_8822c(adapter, HALMAC_CMD_CNSTR_IDLE) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;
...
/**
 * dump_efuse_map_bt_8822c() - dump "BT physical" efuse map
 * @adapter : the adapter of halmac
 * @bank : bt efuse bank
 ...
 */
enum halmac_ret_status
dump_efuse_map_bt_8822c(struct halmac_adapter *adapter,
		       enum halmac_efuse_bank bank, u32 size, u8 *map)
{
	...
	if (cnv_efuse_state_8822c(adapter, HALMAC_CMD_CNSTR_BUSY) !=
	    HALMAC_RET_SUCCESS)
		return HALMAC_RET_ERROR_STATE;

	status = switch_efuse_bank_8822c(adapter, bank);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]switch efuse bank!!\n");
		return status;
	}
	...
	status = read_hw_efuse_8822c(adapter, BT_START_OFFSET, size, map);
	if (status != HALMAC_RET_SUCCESS) {
		bt_switch = 0;
...
/**
 * write_efuse_bt_8822c() - write "BT physical" efuse offset
 * @adapter : the adapter of halmac
 * @offset : offset
 * @value : Write value
 * @map : bt efuse map
 ...
 */
enum halmac_ret_status
write_efuse_bt_8822c(struct halmac_adapter *adapter, u32 offset, u8 value,
		    enum halmac_efuse_bank bank)
{
	...
	status = write_hw_efuse_8822c(adapter, offset + BT_START_OFFSET, value);
	if (status != HALMAC_RET_SUCCESS) {
		bt_switch = 0;
...
/**
 * read_efuse_bt_8822c() - read "BT physical" efuse offset
 * @adapter : the adapter of halmac
 * @offset : offset
 ...
 */
enum halmac_ret_status
read_efuse_bt_8822c(struct halmac_adapter *adapter, u32 offset, u8 *value,
		   enum halmac_efuse_bank bank)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	status =
	read_hw_efuse_8822c(adapter, offset + BT_START_OFFSET, 1, value);
	if (status != HALMAC_RET_SUCCESS) {
		bt_switch = 0;
...
/**
 * get_efuse_available_size_8822c() - get efuse available size
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
get_efuse_available_size_8822c(struct halmac_adapter *adapter, u32 *size)
{
	enum halmac_ret_status status;

	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);

	status = dump_log_efuse_map_8822c(adapter, HALMAC_EFUSE_R_DRV);
...
/**
 * dump_log_efuse_map_8822c() - dump "logical" efuse map
 * @adapter : the adapter of halmac
 * @cfg : dump efuse method
 * Author : Eva
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
dump_log_efuse_map_8822c(struct halmac_adapter *adapter,
			enum halmac_efuse_read_cfg cfg)
{
	u8 *map = NULL;
	...
	*proc_status = HALMAC_CMD_PROCESS_IDLE;
	...
	if (adapter->efuse_map_valid == 1) {
		*proc_status = HALMAC_CMD_PROCESS_DONE;
		...
		if (eeprom_parser_8822c(adapter, adapter->efuse_map, map) !=
		    HALMAC_RET_SUCCESS) {
			PLTFM_FREE(map, size);
...
enum halmac_ret_status
dump_log_efuse_mask_8822c(struct halmac_adapter *adapter,
			 enum halmac_efuse_read_cfg cfg)
{
	u8 *map = NULL;
	u32 size = adapter->hw_cfg_info.eeprom_size;
	...
	*proc_status = HALMAC_CMD_PROCESS_IDLE;
	...
	if (adapter->efuse_map_valid == 1) {
		*proc_status = HALMAC_CMD_PROCESS_DONE;
		...
		if (eeprom_mask_parser_8822c(adapter, adapter->efuse_map, map) !=
		    HALMAC_RET_SUCCESS) {
			PLTFM_FREE(map, size);
			return HALMAC_RET_EEPROM_PARSING_FAIL;
		}
...
/**
 * read_logical_efuse_8822c() - read logical efuse map 1 byte
 * @adapter : the adapter of halmac
 * @offset : offset
 * @value : 1 byte efuse value
 * Author : Eva
 ...
 */
enum halmac_ret_status
read_logical_efuse_8822c(struct halmac_adapter *adapter, u32 offset, u8 *value)
{
	u8 *map = NULL;
	u32 size = adapter->hw_cfg_info.eeprom_size;
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	...
	status = read_log_efuse_map_8822c(adapter, map);
	if (status != HALMAC_RET_SUCCESS) {
...
/**
 * write_log_efuse_8822c() - write "logical" efuse offset
 * @adapter : the adapter of halmac
 * @offset : offset
 * @value : value
 * Author : Eva
 ...
 */
enum halmac_ret_status
write_log_efuse_8822c(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	status = proc_write_log_efuse_8822c(adapter, offset, value);
	if (status != HALMAC_RET_SUCCESS) {
...
/**
 * write_log_efuse_word_8822c() - write "logical" efuse offset word
 * @adapter : the adapter of halmac
 * @offset : offset
 * @value : value
 * Author : Eva
 ...
 */
enum halmac_ret_status
write_log_efuse_word_8822c(struct halmac_adapter *adapter, u32 offset, u16 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	status = proc_write_log_efuse_word_8822c(adapter, offset, value);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]write logical efuse\n");
		return status;
	}
...
/**
 * pg_efuse_by_map_8822c() - pg logical efuse by map
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
pg_efuse_by_map_8822c(struct halmac_adapter *adapter,
		     struct halmac_pg_efuse_info *info,
		     enum halmac_efuse_read_cfg cfg)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	...
	if (info->efuse_map_size != size) {
		if (adapter->intf == HALMAC_INTERFACE_PCIE)
			size = LIM_LOG_SIZE_PCIE_8822C;
		else if (adapter->intf == HALMAC_INTERFACE_USB)
			size = LIM_LOG_SIZE_USB_8822C;
		else if (adapter->intf == HALMAC_INTERFACE_SDIO)
			size = LIM_LOG_SIZE_SDIO_8822C;
		else
	...
	status = proc_pg_efuse_by_map_8822c(adapter, info, cfg);
	if (status != HALMAC_RET_SUCCESS) {
...
/**
 * mask_log_efuse_8822c() - mask logical efuse
 * @adapter : the adapter of halmac
 * @info : efuse map information
 * Author : Eva
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
mask_log_efuse_8822c(struct halmac_adapter *adapter,
		    struct halmac_pg_efuse_info *info)
{
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
	...
	mask_eeprom_8822c(adapter, info);
...
enum halmac_ret_status
switch_efuse_bank_8822c(struct halmac_adapter *adapter,
		       enum halmac_efuse_bank bank)
{
	/*u8 reg_value;
		     ...
		     enum halmac_efuse_read_cfg cfg)
{
	...
	PLTFM_MSG_TRACE("proc_dump_efuse_8822c, efuse_map_valid = %d\n", adapter->efuse_map_valid);
	...
	*proc_status = HALMAC_CMD_PROCESS_SENDING;
	...
	if (cfg == HALMAC_EFUSE_R_AUTO) {
		...
		if (adapter->halmac_state.dlfw_state == HALMAC_DLFW_NONE ||
		    h2c_init == 0)
			status = dump_efuse_drv_8822c(adapter);
		else
			status = dump_efuse_fw_8822c(adapter);
...
enum halmac_ret_status
cnv_efuse_state_8822c(struct halmac_adapter *adapter,
		   ...
		   u8 *map)
{
	...
	//Enable OTP Burst mode
	tmp32 = HALMAC_REG_R32(REG_EFUSE_CTRL_1);
	tmp32 = tmp32 | BIT_EF_BURST;
	HALMAC_REG_W32(REG_EFUSE_CTRL_1, tmp32);
...
enum halmac_ret_status
write_hw_efuse_8822c(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	u8 value_read = 0;
	u32 value32;
	u32 tmp32;
	...
	if (adapter->efuse_auto_check_en == 1) {
		if (read_hw_efuse_8822c(adapter, offset, 1, &value_read) !=
		    HALMAC_RET_SUCCESS)
			return HALMAC_RET_EFUSE_R_FAIL;
		if (value_read != value) {
...
enum halmac_ret_status
eeprom_parser_8822c(struct halmac_adapter *adapter, u8 *phy_map, u8 *log_map)
{
	u8 i;
	u8 value8;
	u8 blk_idx;
	...
	u8 hdr2 = 0;
	u32 eeprom_idx;
	u32 efuse_idx = SEC_CTRL_EFUSE_SIZE_8822C;
...
enum halmac_ret_status
eeprom_mask_parser_8822c(struct halmac_adapter *adapter, u8 *phy_map,
			u8 *log_mask)
{
	u8 i;
	u8 value8;
...
read_log_efuse_map_8822c(struct halmac_adapter *adapter, u8 *map)
{
	...
	if (adapter->efuse_map_valid == 0) {
		...
		status = read_efuse_8822c(adapter, 0, efuse_size, local_map);
		if (status != HALMAC_RET_SUCCESS) {
			PLTFM_MSG_ERR("[ERR]read efuse\n");
			  ...
			  enum halmac_efuse_read_cfg cfg)
{
	...
	u8 *updated_map = NULL;
	u32 map_size, mask_size;
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;

	map_size = adapter->hw_cfg_info.eeprom_size;
	mask_size = map_size >> 4;

	status = compare_version(adapter, info, PG_VER_LEN_8822C);
	if (status != HALMAC_RET_SUCCESS) {
	...
	status = update_eeprom_mask_8822c(adapter, info, updated_mask);
	if (status != HALMAC_RET_SUCCESS) {
	...
	status = check_efuse_enough_8822c(adapter, info, updated_mask);
	if (status != HALMAC_RET_SUCCESS) {
	...
	status = program_efuse_8822c(adapter, info, updated_mask);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]pg efuse\n");
		PLTFM_FREE(updated_mask, mask_size);
		return status;
...
dump_efuse_drv_8822c(struct halmac_adapter *adapter)
{
	...
	PLTFM_MSG_TRACE("dump_efuse_drv_8822c, efuse_map_valid = %d\n", adapter->efuse_map_valid);
	...
	if (adapter->efuse_map_valid == 0) {
		...
		if (read_hw_efuse_8822c(adapter, 0, efuse_size, map) !=
		    HALMAC_RET_SUCCESS) {
			PLTFM_FREE(map, efuse_size);
			return HALMAC_RET_EFUSE_R_FAIL;
		}
...
proc_write_log_efuse_8822c(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	...
	status = write_hw_efuse_8822c(adapter, end, hdr);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_FREE(map, eeprom_size);
		return status;
	}
	...
	if (!bt_switch || offset >= BT_1B_ENTRY_SIZE) {
		status = write_hw_efuse_8822c(adapter, end, hdr2);
		if (status != HALMAC_RET_SUCCESS) {
			PLTFM_FREE(map, eeprom_size);
			return status;
		}
	...
	status = write_hw_efuse_8822c(adapter, end, byte1);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_FREE(map, eeprom_size);
		return status;
	}

	status = write_hw_efuse_8822c(adapter, end + 1, byte2);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_FREE(map, eeprom_size);
		return status;
	}
...
enum halmac_ret_status
read_efuse_8822c(struct halmac_adapter *adapter, u32 offset, u32 size, u8 *map)
{
	if (!map) {
		PLTFM_MSG_ERR("[ERR]malloc map\n");
		return HALMAC_RET_NULL_POINTER;
	...
	if (adapter->efuse_map_valid == 1) {
		...
		if (read_hw_efuse_8822c(adapter, offset, size, map) !=
		    HALMAC_RET_SUCCESS)
			return HALMAC_RET_EFUSE_R_FAIL;
	}
			...
			struct halmac_pg_efuse_info *info, u8 *updated_mask)
{
	...
	status = eeprom_mask_parser_8822c(adapter, adapter->efuse_map, mask_map);
	if (status != HALMAC_RET_SUCCESS) {
		   ...
		   u8 pre_word_en, u32 eeprom_offset)
{
	...
	status = write_hw_efuse_8822c(adapter, efuse_end, hdr);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]write efuse\n");
		return status;
	}

	status = write_hw_efuse_8822c(adapter, efuse_end + 1, hdr2);
	if (status != HALMAC_RET_SUCCESS) {
	...
	for (i = 0; i < 4; i++) {
		if (((pre_word_en >> i) & 0x1) > 0) {
			status = write_hw_efuse_8822c(adapter, efuse_end,
						     *(info->efuse_map +
						     eeprom_offset +
						     (i << 1)));
			if (status != HALMAC_RET_SUCCESS) {
			...
			status = write_hw_efuse_8822c(adapter, efuse_end + 1,
						     *(info->efuse_map +
						     eeprom_offset + (i << 1)
						     + 1));
			if (status != HALMAC_RET_SUCCESS) {
		   ...
		   struct halmac_pg_efuse_info *info, u8 *updated_mask)
{
	...
	for (i = 0; i < info->efuse_map_size; i += 8) {
		...
		if (pre_word_en > 0) {
			status = proc_pg_efuse_8822c(adapter, info,
						    word_en,
						    pre_word_en,
						    eeprom_offset);
			if (status != HALMAC_RET_SUCCESS) {
...
enum halmac_ret_status
get_efuse_data_8822c(struct halmac_adapter *adapter, u8 *buf, u32 size)
{
	u8 seg_id;
	u8 seg_size;
	u8 seq_num;
	...
	seg_offset = (u32)(seg_id * adapter->efuse_seg_size);

	PLTFM_MUTEX_LOCK(&adapter->efuse_mutex);
	if (efuse_c2h_part == EFUSE_C2H_PART_1) {
		PLTFM_MEMCPY(adapter->efuse_map + seg_offset,
			     buf + C2H_DATA_OFFSET_88XX, seg_size);
	} else {
		seg_offset += EFUSE_PART_1_END;
		if ((seg_offset + seg_size) <= efuse_size) {
			PLTFM_MEMCPY(adapter->efuse_map + seg_offset,
				     buf + C2H_DATA_OFFSET_88XX, seg_size);
		} else if (seg_offset < efuse_size &&
			   (seg_offset + seg_size) > efuse_size){
			PLTFM_MEMCPY(adapter->efuse_map + seg_offset,
				     buf + C2H_DATA_OFFSET_88XX,
				     efuse_size - seg_offset);
		}
	}
	PLTFM_MUTEX_UNLOCK(&adapter->efuse_mutex);
	...
	if (EFUSE_DATA_GET_END_SEGMENT(buf) == 0) {
		...
		if (efuse_c2h_part == EFUSE_C2H_PART_1) {
			efuse_c2h_part = EFUSE_C2H_PART_2;

			PLTFM_FREE(map, eeprom_size);
			return HALMAC_RET_SUCCESS;
		} else {
			efuse_c2h_part = EFUSE_C2H_PART_1;
		}
	}
	...
	if ((enum halmac_h2c_return_code)fw_rc == HALMAC_H2C_RETURN_SUCCESS) {
		...
		if (adapter->evnt.log_efuse_map == 1) {
			if (eeprom_parser_8822c(adapter, adapter->efuse_map,
		...
		if (adapter->evnt.log_efuse_mask == 1) {
			if (eeprom_mask_parser_8822c(adapter, adapter->efuse_map,
						    map)
						    != HALMAC_RET_SUCCESS) {
				PLTFM_FREE(map, eeprom_size);
				return HALMAC_RET_EEPROM_PARSING_FAIL;
...
enum halmac_ret_status
get_dump_phy_efuse_status_8822c(struct halmac_adapter *adapter,
			       enum halmac_cmd_process_status *proc_status,
			       u8 *data, u32 *size)
{
	u8 *map = NULL;
...
enum halmac_ret_status
get_dump_log_efuse_status_8822c(struct halmac_adapter *adapter,
			       enum halmac_cmd_process_status *proc_status,
			       u8 *data, u32 *size)
{
	u8 *map = NULL;
...
enum halmac_ret_status
get_dump_log_efuse_mask_status_8822c(struct halmac_adapter *adapter,
				    enum halmac_cmd_process_status *proc_status,
				    u8 *data, u32 *size)
{
	u8 *map = NULL;
...
enum halmac_ret_status
get_h2c_ack_phy_efuse_8822c(struct halmac_adapter *adapter, u8 *buf, u32 size)
{
	u8 seq_num = 0;
	u8 fw_rc;
	struct halmac_efuse_state *state = &adapter->halmac_state.efuse_state;
...
/**
 * write_wifi_phy_efuse_8822c() - write wifi physical efuse
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
write_wifi_phy_efuse_8822c(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	enable_efuse_sw_pwr_cut(adapter, 1);
	status = write_hw_efuse_8822c(adapter, offset, value);
	if (status != HALMAC_RET_SUCCESS) {
...
/**
 * read_wifi_phy_efuse_8822c() - read wifi physical efuse
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
read_wifi_phy_efuse_8822c(struct halmac_adapter *adapter, u32 offset, u32 size,
			 u8 *value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	PLTFM_MSG_TRACE("read_wifi_phy_efuse_8822c, efuse_map_valid = %d\n",
			adapter->efuse_map_valid);
	...
	status = read_hw_efuse_8822c(adapter, offset, size, value);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]read hw efuse\n");
		return status;
	}
				    ...
				    u8 is_write)
{
	...
	HALMAC_REG_W16(REG_SYS_ISO_CTRL, val16 | BIT_PWC_EV2EF_S_8822C);
	PLTFM_DELAY_US(1000);

	val16 = HALMAC_REG_R16(REG_SYS_ISO_CTRL);
	HALMAC_REG_W16(REG_SYS_ISO_CTRL, val16 | BIT_PWC_EV2EF_B_8822C);
				     ...
				     u8 is_write)
{
	...
	value16 = HALMAC_REG_R16(REG_SYS_ISO_CTRL);
	HALMAC_REG_W16(REG_SYS_ISO_CTRL, value16 & ~(BIT_PWC_EV2EF_B_8822C));

	PLTFM_DELAY_US(1000);

	value16 = HALMAC_REG_R16(REG_SYS_ISO_CTRL);
	HALMAC_REG_W16(REG_SYS_ISO_CTRL, value16 & ~(BIT_PWC_EV2EF_S_8822C));
...
/**
 * switch_ctrl_reg_8822c() - switch efuse control reg to 0x1030 or 0x30
 * @adapter : the adapter of halmac
 * Author : Eva
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
switch_ctrl_reg_8822c(struct halmac_adapter *adapter, u8 switch_reg)
{
